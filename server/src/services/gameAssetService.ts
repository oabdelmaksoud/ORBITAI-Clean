...
class GameAssetService {
  ...
  async generateGameAssetPack(request: GenerateAssetRequest): Promise<IAssetGenerationJob> {
    ...
    const assetList =
      request.assetList || this.determineAssetList(request.gameType, request.genre, request.theme);
  ...
  private async processGenerationJob(jobId: string): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < job.assetList.length; i++) {
        ...
        try {
          logger.info(
            `[GameAssetService] Generating asset ${i + 1}/${job.totalAssets}: ${assetSpec.description}`
          );
          ...
          logger.error(
            `[GameAssetService] Failed to generate asset: ${assetSpec.description}`,
            error
          );
      ...
      logger.info(
        `[GameAssetService] Job ${jobId} completed: ${job.completedAssets}/${job.totalAssets} successful`
      );
    } catch (error: unknown) {
  ...
  async generate3DAsset(
    ...
    try {
      ...
      if ((error as any).response?.status === 429 || (error as any).code === 'ECONNABORTED') {
      ...
      throw new Error(
        `3D asset generation failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async generate2DAsset(
    ...
    try {
      ...
      throw new Error(
        `2D asset generation failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private async pollTripoTask(taskId: string, apiKey: string, maxAttempts = 60): Promise<any> {
    for (let i = 0; i < maxAttempts; i++) {
      const response = await axios.get(`${TRIPO_API_URL}/task/${taskId}`, {
   ...
   */
  private async generateThumbnail(_assetPath: string, _format: string): Promise<string> {
   ...
   */
  private determineAssetList(gameType: '2D' | '3D', _genre: string, theme: string): any[] {
    const assetList: any[] = [];
