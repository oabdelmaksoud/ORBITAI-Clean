...
// DOM types used in browser-evaluated code strings (not actually executed in Node)
...
export interface TestScenario {
  id: string;
  type: 'click' | 'input' | 'keyboard' | 'hover' | 'scroll' | 'wait' | 'load';
  target: string;
  selector?: string;
  description: string;
  ...
  category?: 'setup' | 'gameplay' | 'validation' | 'edge_case' | string;
}
...
class CUAService {
    ...
    this.frameStreamInterval = setInterval(async () => {
      try {
        ...
        if (frameCount <= 3 || frameCount % 30 === 0) {
          logger.info(
            `[CUA] Frame ${frameCount} captured for session ${sessionId}, size: ${Math.round(frame.length / 1024)}KB`
          );
        }
   ...
   */
  private async validateScenarioSelectors(
    page: Page,
    scenarios: TestScenario[]
    ...
    for (const scenario of scenarios) {
      ...
      try {
        ...
        if (isVisible) {
          ...
          logger.debug(
            `[CUA] Skipping scenario "${scenario.target}" - selector not found: ${scenario.selector}`
          );
        }
      } catch (error: unknown) {
        logger.debug(
          `[CUA] Selector validation failed for "${scenario.target}": ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
...
Generate 8-15 tests with CORRECT dependencies. Setup tests MUST come first!`;
      ...
      logger.info(
        `[CUA] Generated ${scenarios.length} AI-powered test scenarios (sorted by phase)`
      );
      return scenarios;
    } catch (error: unknown) {
      logger.warn(
        `[CUA] AI analysis failed, falling back to rule-based: ${error instanceof Error ? error.message : String(error)}`
      );
      return null;
    }
  }
  ...
  async generateTestScenarios(page: Page, htmlContent?: string): Promise<TestScenario[]> {
    ...
    if (htmlContent) {
      ...
      if (aiScenarios && aiScenarios.length > 0) {
        ...
        if (validatedScenarios.length > 0) {
          logger.info(
            `[CUA] Using AI-generated scenarios: ${validatedScenarios.length}/${aiScenarios.length} validated`
          );
          return validatedScenarios;
        }
    ...
    const analysis = await page.evaluate(() => {
      ...
      const cards = Array.from(
        document.querySelectorAll('[class*="card"], .card, [class*="product"], [class*="item"]')
      );
      ...
      const hasLogin =
        bodyText.includes('login') || bodyText.includes('sign in') || bodyText.includes('password');
      const hasDashboard =
      ...
      const hasGame =
        ...
        bodyText.includes('game') ||
        bodyText.includes('start');

      return {
        buttons: buttons.map((b, i) => ({
          text: (b as any).textContent?.trim() || `Button ${i + 1}`,
          id: (b as any).id || null,
          className: (b as any).className || null,
          visible: (b as any).offsetParent !== null,
          type: (b as any).type || 'button',
        })),
        inputs: inputs.map(inp => ({
          type: (inp as HTMLInputElement).type || 'text',
          placeholder: (inp as HTMLInputElement).placeholder || null,
          id: (inp as any).id || null,
          name: (inp as HTMLInputElement).name || null,
          visible: (inp as any).offsetParent !== null,
        ...
        selects: selects.map(sel => ({
          ...
          visible: (sel as any).offsetParent !== null,
          optionCount: (sel as HTMLSelectElement).options?.length || 0,
        })),
        checkboxes: checkboxes.map(cb => ({
          id: (cb as any).id || null,
          type: (cb as HTMLInputElement).type,
          visible: (cb as any).offsetParent !== null,
        })),
        links: links
          .filter(l => (l as any).offsetParent !== null)
          .map(l => ({
            text: (l as any).textContent?.trim() || '',
            href: (l as any).getAttribute('href') || '',
          })),
        hasCanvas: !!canvas,
    ...
    if (projectType === 'form' || analysis.inputs.length > 0) {
      ...
      for (const input of visibleInputs.slice(0, 4)) {
        // Limit to 4 inputs
      ...
      switch (scenario.type) {
        case 'click':
          if (scenario.selector) {
            ...
            if (!isVisible) {
              throw new Error(
                `Element not found: ${scenario.selector} - "${scenario.target}" does not exist on page`
              );
            }
            ...
            const clickValidation = this.validateStateChange(
              clickStateBefore,
              clickStateAfter,
              scenario.target
            );
            if (clickValidation.changed) {
              logger.info(
                `[CUA] Click on "${scenario.target}" validated: ${clickValidation.details}`
              );
            } else {
              logger.debug(
                `[CUA] Click on "${scenario.target}" - no state change detected (may be expected)`
              );
            }
          } else {
        ...
        case 'input':
          if (scenario.selector) {
            ...
            if (!isVisible) {
              throw new Error(
                `Input element not found: ${scenario.selector} - "${scenario.target}" does not exist on page`
              );
            }
        ...
        case 'keyboard':
          ...
          logger.debug(
            `[CUA] State before keyboard action: canvas=${stateBefore.canvasHash}, score=${stateBefore.score}`
          );
          ...
          logger.debug(
            `[CUA] State after keyboard action: canvas=${stateAfter.canvasHash}, score=${stateAfter.score}`
          );
        ...
        case 'hover':
          if (scenario.selector) {
            ...
            if (!isVisible) {
              throw new Error(
                `Hover element not found: ${scenario.selector} - "${scenario.target}" does not exist on page`
              );
            }
            const box = await element.boundingBox();
            if (!box) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          logger.info(
            'Timeout waiting for React/Bootstrapper to load - proceeding to content check'
          );
        }
        ...
        try {
          ...
          logger.info('Timeout waiting for content render - checking for errors');
        }
        ...
        const pageValidation = await page.evaluate(() => {
          ...
          // const _bodyHtml = body.innerHTML?.trim() || '';
          ...
          const hasInteractiveElements =
            body.querySelectorAll('button, input, a, [role="button"]').length > 0;
          ...
          const errorDisplay = body.querySelector(
            '.orbitai-bootstrap-error, [data-error-type="bootstrap"], .react-error-boundary, #error-container'
          );
          ...
          const hasBootstrapError =
            !!errorDisplay ||
            ((errorH1Text === 'Load Error' ||
              errorH1Text === 'Application Error' ||
              errorH1Text === 'Runtime Error') &&
          ...
          if (!hasContent && !hasInteractiveElements) {
            ...
            logger.info('[CUA Debug] Blank page detected. InnerHTML start:', debugHtml);
        ...
        for (const scenario of scenarios) {
          ...
          if (scenario.dependsOn && scenario.dependsOn.length > 0) {
            ...
            if (unmetDependencies.length > 0) {
              ...
              logger.warn(
                `[CUA] Skipping "${scenario.target}" - dependencies not met: ${unmetDependencies.join(', ')}`
              );
          ...
          if (scenario.prerequisiteCheck) {
            try {
              ...
              const isPrereqVisible = await prereqElement
              ...
              if (!isPrereqVisible) {
                ...
                logger.warn(
                  `[CUA] Skipping "${scenario.target}" - prerequisite element not visible: ${scenario.prerequisiteCheck}`
                );
                completedScenarios.set(scenario.id, 'skipped');
                result.failedCount!++;
                this.emit('cua:scenario:result', { sessionId, scenario });
          ...
          if (scenario.status === 'failed' && scenario.phase === 1) {
            ...
            logger.info(
              `[CUA Auto-Fix] Phase 1 scenario "${scenario.target}" failed, attempting inline fix...`
            );
            ...
            try {
              ...
              const fixResult = await cuaLLMService.generatePrototypeFix(currentHtml, [
                {
                  scenario: scenario.target,
              ...
              if (fixResult.success && fixResult.fixedHtml !== currentHtml) {
                ...
                if ((scenario.status as string) === 'passed') {
      ...
      result.summary = `✗ Test failed: ${error instanceof Error ? error.message : String(error)}`;
      result.totalDuration = Date.now() - startTime;
    }
  ...
  > {
    ...
    while (attempts < maxRetries + 1) {
      ...
      const attemptLabel =
        attempts === 1 ? originalSessionId : `${originalSessionId}-fix${attempts - 1}`;

      logger.info(
        `[CUA Auto-Fix] Attempt ${attempts}/${maxRetries + 1} for session ${attemptLabel}`
      );
      ...
      if (attempts <= maxRetries) {
        ...
        // Generate fix
        const fixResult = await cuaLLMService.generatePrototypeFix(currentHtml, failedTests);

        if (fixResult.success && fixResult.fixedHtml !== currentHtml) {
          logger.info(
            `[CUA Auto-Fix] Generated fix with ${fixResult.changes.length} changes: ${fixResult.explanation}`
          );
          ...
          try {
            ...
            logger.info(
              `[CUA Auto-Fix] Recorded ${failedTests.length} learnings from successful fix`
            );
          } catch (learnErr: any) {
