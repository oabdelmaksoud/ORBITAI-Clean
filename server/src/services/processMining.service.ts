...
// Task type used for process mining analysis
interface Task {
  id: string;
  title?: string;
  status?: string;
...
class ProcessMiningService {
  ...
  private extractTaskSequences(
    ...
    const agentTasks = tasks.filter(
      t => t.assignedAgent?.role === agentRole || t.assignedAgent?.agentRole === agentRole
    );
        ...
        if (currentSequence.length > 0) {
          sequences.push({
            ...
            endTime: currentSequence[currentSequence.length - 1].createdAt
              ? new Date(currentSequence[currentSequence.length - 1].createdAt as any)
              : new Date(),
  ...
  private discoverPatterns(
    ...
    sequences.forEach(seq => {
      ...
      if (!patternMap.has(pattern)) {
        patternMap.set(pattern, {
          sequences: [],
          pattern: seq.tasks.map(t => t.title || 'unknown'),
        });
      }
    ...
    patternMap.forEach((data, _patternKey) => {
      ...
      const durations = data.sequences.map(seq => {
        ...
        const end = seq.tasks[seq.tasks.length - 1].createdAt
          ? new Date(seq.tasks[seq.tasks.length - 1].createdAt as any).getTime()
          : Date.now();
        return end - start;
      });
  ...
  private detectBottlenecks(
    ...
    patterns.forEach(pattern => {
      pattern.steps.forEach((step, index) => {
        ...
        sequences.forEach(seq => {
          if (seq.tasks.length > index + 1) {
            ...
            if (currentTask.createdAt && nextTask.createdAt) {
              const wait =
                new Date(nextTask.createdAt).getTime() -
        ...
        if (averageWaitTime > 3600000)
          impact = 'high'; // > 1 hour
        else if (averageWaitTime > 600000) impact = 'medium'; // > 10 minutes

        if (averageWaitTime > 0 || step.averageDuration > 300000) {
          // > 5 minutes
          bottlenecks.push({
   ...
   */
  // @ts-ignore TS6133
  private async _performDeepLearningAnalysis(
    taskSequences: Array<{
      sequenceId: string;
      tasks: Task[];
      startTime: number;
  ...
  private detectSequentialDependencies(
    ...
    dependencies.forEach((toMap, from) => {
      toMap.forEach((count, to) => {
        ...
        if (strength > 0.5) {
          // Strong dependency (>50% of sequences)
