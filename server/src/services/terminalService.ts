...
// execFile is used instead of exec to prevent shell injection attacks

// Security: Safe root directories for CWD validation
const SAFE_ROOT_PATHS = ['/tmp', '/var/tmp'];
const SAFE_ROOT_PATH_PATTERNS = [/\/orbitai\b/i];

// Security: Command whitelist - only allow safe commands
// NOTE: curl and wget are intentionally excluded (they are in BLOCKED_COMMANDS)
// to prevent data exfiltration and arbitrary network requests
...
export async function executeCommand(
  ...
  // Security: Resolve CWD to absolute path and validate against safe roots
  const resolvedCwd = path.resolve(cwd);
  const isSafeRoot = SAFE_ROOT_PATHS.some(safePath => resolvedCwd.startsWith(safePath));
  const isSafePattern = SAFE_ROOT_PATH_PATTERNS.some(pattern => pattern.test(resolvedCwd));
  const safeCwd = isSafeRoot || isSafePattern ? resolvedCwd : '/tmp';

  // Parse command into executable and arguments for execFile
