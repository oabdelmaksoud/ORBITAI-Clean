...
import {
  UNITY_2D_MOVEMENT_TEMPLATE,
  UNITY_2D_MOVEMENT_CONFIG,
} from '../templates/mechanics/unity/PlayerController.template.js';
import {
  GODOT_2D_MOVEMENT_TEMPLATE,
  GODOT_2D_MOVEMENT_CONFIG,
} from '../templates/mechanics/godot/player_controller.template.js';
import {
  PHASER_2D_MOVEMENT_TEMPLATE,
  PHASER_2D_MOVEMENT_CONFIG,
} from '../templates/mechanics/phaser/PlayerController.template.js';
import {
  UNITY_COMBAT_TEMPLATE,
  UNITY_COMBAT_CONFIG,
} from '../templates/mechanics/unity/CombatSystem.template.js';
import {
  UNITY_ENEMY_AI_CODE,
  UNITY_ENEMY_AI_CONFIG,
} from '../templates/mechanics/unity/EnemyAI.template.js';
import {
  GODOT_ENEMY_AI_CODE,
  GODOT_ENEMY_AI_CONFIG,
} from '../templates/mechanics/godot/enemy_ai.template.js';
import {
  PHASER_ENEMY_AI_CODE,
  PHASER_ENEMY_AI_CONFIG,
} from '../templates/mechanics/phaser/EnemyAI.template.js';
import {
  UNITY_XP_SYSTEM_CODE,
  UNITY_XP_SYSTEM_CONFIG,
} from '../templates/mechanics/unity/XPSystem.template.js';
import {
  UNITY_INVENTORY_SYSTEM_CODE,
  UNITY_INVENTORY_SYSTEM_CONFIG,
} from '../templates/mechanics/unity/InventorySystem.template.js';
import {
  UNITY_QUEST_SYSTEM_CODE,
  UNITY_QUEST_SYSTEM_CONFIG,
} from '../templates/mechanics/unity/QuestSystem.template.js';
...
class GameMechanicsService {
  ...
  async generateMechanics(request: MechanicsRequest): Promise<GeneratedMechanics> {
    try {
      ...
      throw new Error(
        `Mechanics generation failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private async analyzeWithLLM(request: MechanicsRequest): Promise<GameAnalysis> {
    try {
      ...
      // @ts-ignore TS6133 - LLMRouterInstance is used below
      const { llmRouter: LLMRouterInstance } = await import('./llm/LLMRouter.js');

      const prompt = GAME_ANALYSIS_PROMPT.replace('{{description}}', request.gameDescription);

      const response = await (LLMRouterInstance as any).generateResponse({
      ...
      logger.warn(
        '[GameMechanics] LLM analysis failed, falling back to patterns:',
  ...
  private async analyzeWithPatterns(request: MechanicsRequest): Promise<GameAnalysis> {
    ...
    const is2DPlatformer =
      description.includes('platformer') ||
      (description.includes('2d') &&
        (description.includes('jump') || description.includes('platform')));

    const hasEnemies =
      description.includes('enemy') ||
      description.includes('enemies') ||
  ...
  private getFilenameForTemplate(template: MechanicsTemplate): string {
    ...
    switch (template.engine) {
      ...
      default:
        return 'PlayerController.txt';
    }
  }

  private createMechanicsSummary(
    templates: MechanicsTemplate[],
    ...
    return {
      ...
      details: {
        ...
        progressionType: analysis.requiredMechanics.progression?.type as any,
      },
    };
  }
