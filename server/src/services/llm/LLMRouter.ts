...
export class LLMRouter {
   ...
   */
  private rateLimitBackoff = new Map<
    string,
    { attempts: number; nextRetry: number; timestamp: number }
  >();
  private rateLimitCleanupInterval: ReturnType<typeof setInterval> | null = null;

  constructor() {
    // Clean up stale rateLimitBackoff entries every 10 minutes
    this.rateLimitCleanupInterval = setInterval(
      () => {
        this.cleanupRateLimitBackoff();
      },
      10 * 60 * 1000
    );
  }
  ...
  /**
   * Remove rateLimitBackoff entries older than 1 hour
   */
  private cleanupRateLimitBackoff(): void {
    const oneHourAgo = Date.now() - 60 * 60 * 1000;
    for (const [key, value] of this.rateLimitBackoff) {
  ...
  /**
   * Clean up resources (e.g., intervals) when shutting down
   */
  destroy(): void {
  ...
  private async handleRateLimit(
    ...
    const backoff = this.rateLimitBackoff.get(key) || {
      attempts: 0,
      nextRetry: 0,
      timestamp: Date.now(),
    };
  ...
  async executeWithFallback(params: {
    ...
    try {
      ...
      if (routingContext?.userId) {
        const userActiveModels = modelRegistry.getActiveModelsForUser(routingContext.userId);
    ...
    try {
      ...
      if (activeModels.length > 0) {
        ...
        try {
          ...
          const fbApiError = toApiError(fallbackError);
          throw new Error(`Streaming failed with fallback: ${fbApiError.message}`);
        }
      } else {
  ...
  private async executeWithProvider(
    ...
    routingContext?: RoutingContext,
    _requestType: string = 'chat',
    _contextType: 'wizard' | 'workspace' | 'other' = 'other'
    ...
    const circuitState = llmCircuitBreaker.getState(provider);
    if (circuitState === 'OPEN') {
      logger.warn(`[LLMRouter] Circuit breaker OPEN for ${provider}, skipping provider`);
    ...
    try {
      if (provider === 'gemini') {
        ...
        llmCircuitBreaker.recordSuccess('gemini');
        return response;
      } else if (provider === 'openai') {
        const serviceConfig: any = {
          systemInstruction: config?.systemInstruction,
