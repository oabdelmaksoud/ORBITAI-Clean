...
export class PrototypeValidationService {
  ...
  validateArchitectureDiagram(diagram: string): ArchitectureValidationResult {
    ...
    if (!diagram || diagram.trim().length < 50) {
      issues.push({
        ...
        suggestion:
          'Generate a complete C4 Container diagram with at least 2 containers and relationships',
      });
      return {
        isValid: false,
        issues,
    ...
    // Count containers
    const containerMatches = cleanDiagram.match(
      /(Container|System|ContainerDb|Container_Ext|System_Ext)\(/gi
    );
    ...
    const hasTechAnnotations =
      /Container\([^,]+,[^,]+,[^,]+,"[^"]+"\)/i.test(cleanDiagram) ||
      cleanDiagram.includes('Technology') ||
      (cleanDiagram.match(/"[A-Za-z0-9\s]+"/g)?.length ?? 0) > 0;
    ...
    const hasValidFunctions = lines.some(line =>
      /^(Container|System|Person|System_Ext|Container_Ext|Rel|SystemBoundary|ContainerBoundary|Person_Ext|UpdateElementStyle|UpdateRelStyle|UpdateLayoutConfig)\(/i.test(
        line
      )
    );
  ...
  validateWireframeCode(code: string): WireframeValidationResult {
    ...
    // const _hasBodyTag = code.includes('<body') || code.includes('<BODY');
    // const _hasHeadTag = code.includes('<head') || code.includes('<HEAD');
    ...
    const hasReactCDN =
      code.includes('unpkg.com/react') || code.includes('cdnjs.cloudflare.com/react');
    ...
    const hasSemanticHTML = /<(header|nav|main|article|section|footer|button|form|input)/i.test(
      code
    );
    ...
    // const _hasAsyncScripts = code.includes('async') || code.includes('defer');
    // const _hasOptimizedImages = code.includes('width=') && code.includes('height=');
    ...
    const hasDangerousPatterns =
      /onclick=["']javascript:/i.test(code) || /href=["']javascript:/i.test(code);
    ...
    // const _hasStateManagement = code.includes('useState') || code.includes('useReducer') || code.includes('state');
  ...
  validateVisualDesign(code: string): VisualDesignValidationResult {
    ...
    const hasSemanticHTML =
      /<(header|nav|main|article|section|footer|button|form|input|label)/i.test(code);
    // const _hasAltText = /<img[^>]+alt=/i.test(code);
