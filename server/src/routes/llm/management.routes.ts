  ...
  try {
    ...
    if (
      ...
      const processedResult = await functionCallProcessor.processWithFunctionCalls(
        responseWithFunctionCalls as any,
        projectState,
        mcpServers || []
      );
      ...
      res.json({
        success: true,
        output: (processedResult as any).output,
        resources: result.resources || [], // URLs from Google Search grounding
        functionCalls: (processedResult as any).functionCalls,
        modelUsed: result.modelUsed,
        provider: result.provider,
      });
      return;
    ...
    const outputText = result.text || (result as any).content || '';
...
router.post('/generate-theme', async (req: AuthRequest, res, _next) => {
  try {
    ...
    // const _isWebsite = projectType === 'website' || projectType === 'web-app';
    // const _isMobile = projectType === 'mobile-app';
...
router.post('/orchestrate', async (req: AuthRequest, res, _next) => {
  try {
    // @ts-ignore TS6133
    const { phase, description, completedTasks, useInternet, _mcpServers, maxTasks, agents } =
      req.body;
...
Keep the research concise and focused on actionable insights for task generation.`;
    ...
    const { enhanceOrchestrationPrompt } =
      await import('../../services/promptEngineering.service.js');
