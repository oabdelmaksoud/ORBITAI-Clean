...
import {
  ...
  FeatureRequest,
} from '../middleware/featureCheck.js';
  ...
  try {
    ...
    try {
      ...
      if (authHeader && authHeader.startsWith('Bearer ')) {
        ...
        const jwtSecret = (envConfig as any).config?.jwtSecret || (envConfig as any).JWT_SECRET;
        if (!jwtSecret) {
  ...
  try {
    ...
    if (canViewAllProjects) {
      ...
      projects = await Project.find({})
        .sort({ lastModified: -1 })
        .select(
          'name description currentPhase currentSprint methodology lastModified createdAt userId'
        )
        .lean();
  ...
  try {
    ...
    if (!project) {
      ...
      if (anyProject) {
        ...
        logger.warn(
          `Project access denied: ${projectId} (exists: yes, owner: ${anyProject.userId}, requester: ${userId}, role: ${userRole}, canViewAll: ${canViewAllProjects}, isSample: ${anyProject.isSample})`
        );
        throw new AppError('Project not found or access denied', 404);
      } else {
        // Project doesn't exist at all
        logger.warn(
          `Project not found: ${projectId} (user: ${userId}, role: ${userRole}, canViewAll: ${canViewAllProjects}) - Project does not exist in database`
        );
    ...
    try {
      ...
      if (projectData.description && projectData.description.trim().length > 0) {
        try {
          ...
          logger.info(
            `Auto-assigned methodology: ${projectData.methodology} (${projectData.estimatedSprints} sprints) for project: ${projectData.name}`
          );
        } catch (sdlcError: any) {
      ...
      if (projectData.description && projectData.description.trim().length > 0) {
        try {
          const { standardsMatchingService } =
            await import('../services/standardsMatching.service.js');
          ...
          logger.info(
            `Auto-assigned ${enrolled.length} standards for project: ${projectData.name}`,
            {
              standards: enrolled,
      ...
      if (projectData.description && projectData.description.trim().length > 0) {
        try {
          const { projectArchitectureAnalyzer } =
            await import('../services/projectArchitectureAnalyzer.service.js');
      ...
      try {
        ...
        if (project) {
          ...
          const sdlcConfig = await sdlcMatchingService.autoConfigureSDLC({
            ...
            category: req.body.category || (project as any).category,
          ...
          logger.info(
            `Auto-updated methodology: ${updateData.methodology} (${updateData.estimatedSprints} sprints) for project: ${project.name}`
          );
        }
      } catch (sdlcError: any) {
    ...
    if (req.body.description && req.body.description.trim().length > 0) {
      try {
        ...
        if (project) {
          ...
          logger.info(
            `Auto-updated standards: ${enrolled.length} new standards for project: ${project.name}`,
            {
...
router.post('/:id/share', async (req: FeatureRequest, res, next) => {
  try {
    ...
    project.shareTokens.push(shareLink as any);
    await project.save();
...
router.post(
  '/import',
  checkFeatureAccess('project_import'),
  async (req: FeatureRequest, res, next) => {
    try {
      const userId = req.user!.id;
      ...
      const importData: any = {
        ...
        selectedStandards: Array.isArray(projectData.selectedStandards)
          ? projectData.selectedStandards
          : [],
        ...
        project = await Project.findOneAndUpdate(
          { _id: projectData.id, userId: userId },
          {
            ...
            tasks: [
              ...
              ...importData.tasks.filter(
                (t: any) => !existingProject.tasks.some((et: any) => et.id === t.id)
              ),
            ],
            artifacts: [
              ...(existingProject.artifacts || []),
              ...importData.artifacts.filter(
                (a: any) => !existingProject.artifacts.some((ea: any) => ea.id === a.id)
              ),
            ],
            logs: [...(existingProject.logs || []), ...importData.logs],
            selectedStandards: [
              ...new Set([
                ...(existingProject.selectedStandards || []),
                ...importData.selectedStandards,
              ]),
            ],
            ...
            mcpServers: [
              ...
              ...importData.mcpServers.filter(
                (m: any) => !existingProject.mcpServers.some((em: any) => em.id === m.id)
              ),
            ],
            lastModified: new Date(),
          },
    ...
    if (architecture.needsAdminPanel) {
      const adminTask = {
        ...
        tags: [
          ...
          'dashboard',
          'control-panel',
          architecture.adminPanelType?.toLowerCase().replace(' ', '-') || 'web-dashboard',
        ],
        // Initialize evaluation with pending status (will be calculated when task is executed)
        evaluation: {
          score: 0,
...
router.post(
  '/:id/export-report',
  checkFeatureAccess('export_reports'),
  async (req: FeatureRequest, res, next) => {
    try {
      const projectId = req.params.id;
      const userId = req.user!.id;
      const { format, includeTasks, includeArtifacts, includeLogs, includeAgents, title } =
        req.body;
...
router.post('/:id/package', authenticateToken, async (req: AuthRequest, res, next) => {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${packageResult.metadata.projectName.replace(/\s+/g, '-')}-package.zip"`
    );
